# Migraine World Summit Podcast Generator
...
This project provides a more convenient way to listen to Migraine World Summit talks by converting them into a private podcast feed. While the summit content is available through the official website, accessing it through a podcast player offers several advantages:
...
```bash
python fetch_talk_details.py --year 2026
```
...
```bash
python create_podcast_feed.py --year 2026
```
This script will:
- Read the talk details from `talks.json`
- Generate a podcast feed in XML format
- Save the feed to `mws2026_podcast.xml`
...
1. **Amazon S3 Bucket (Recommended)**:
   ...
   - Upload the XML file
   - The URL will be in the format: `https://<bucket-name>.s3.<region>.amazonaws.com/mws<year>_podcast.xml`
...
- `mws<year>_podcast.xml`: The generated podcast feed in XML format (generated by `create_podcast_feed.py`)
...
- Both scripts accept a `--year` argument (e.g., `--year 2026`) to specify the summit year
